...
Mandatory stack: React + Redux, FastAPI, LangGraph, Groq (`openai/gpt-oss-20b` primary, `openai/gpt-oss-120b` for heavier reasoning), Postgres/MySQL, Google Inter font.

**Model substitution note**: the assignment doc names `gemma2-9b-it` and `llama-3.3-70b-versatile`. As of 2026-07-24, Groq has retired `gemma2-9b-it` (shut down Oct 8, 2025) and scheduled `llama-3.3-70b-versatile` for shutdown Aug 16, 2026 — too close to this build's timeline to rely on. Substituted with Groq's own recommended replacements, `openai/gpt-oss-20b` (fast/cheap tier) and `openai/gpt-oss-120b` (reasoning tier), same free-tier rate limits, no credit card needed. Document this swap explicitly in the README as a deliberate, dated engineering decision.
...
2. **`classify_intent`** — decide `new_extraction` / `correction` / `question`. Cheap heuristic first: if `current_fields` is empty or an attachment is present → `new_extraction`; otherwise ask `openai/gpt-oss-20b` to classify the message against the current field snapshot. Keep this fast and cheap — it runs on every single turn.
...
- Why `openai/gpt-oss-20b` for extraction/intent-classification (fast/cheap, structured JSON tasks) vs `openai/gpt-oss-120b` for the risk-assessment narrative (needs more reasoning depth). (Substituted for the assignment's named `gemma2-9b-it`/`llama-3.3-70b-versatile`, both retired or being retired by Groq — see the model substitution note in Section 1.)
